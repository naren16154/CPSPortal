package com.agilent.cps.utils;

public class Constants {
	public static String browser = "CHROME";
	public static String isrcserver = "false";
	public static String port = "4444";
	public static String chromeDriver = "lib/chromedriver.exe";
	public static String firefoxDriver = "lib/geckodriver.exe";
	public static String ieDriver = "lib/IEDriverServer.exe";
	public static String edgeDriver = "";
	public static String pageLoadTimeout = "180";
	public static String implicitWait = "1";
	public static String screenshotPath = "snapshots/";
	
	public static String logName = "AdfLog";
	public static String timeStamp = "fasle";
	public static String success_robot = "false";
	public static String success_scroll = "false";
	public static String failure_robot = "false";
	public static String failure_scroll = "false";
	public static String error_robot = "false";
	public static String error_scroll = "true";
	public static String isRCServer = "false";
	public static String templateFile = "";
	public static String openLogFile = "false";
	
	public static final String key_Y = "Y";
	public static final String key_Yes = "YES";
	public static final String key_N = "N";
	public static final String key_No = "NO";
	public static final String key_Check = "CHECK";
	public static final String key_Uncheck = "UNCHECK";
	public static final String key_Click = "CLICK";
	public static final String key_True = "TRUE";
	public static final String key_False = "FALSE";
}
